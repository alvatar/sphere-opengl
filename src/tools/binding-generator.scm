(define gl-es2-api #<<end-of-string
...
(define (generate-function-binding declaration)
    ...
    (let recur ((tok list-tokens)
      ...
      (cond ((null? tok) '()) ; This is just a lazy (and completely wrong) way
            ((or (string=? "" (car tok))
                 ...
                 (string=? ";" (car tok))
                 (string=? ");" (car tok))
                 (string=? "GL_APIENTRY" (car tok))
                 (string=? "GL_APICALL" (car tok)))
             (recur (cdr tok) state))
            (else
             (let* ((head (car tok))
                    (tail (cdr tok))
               ...
               (case state
                 ((searching-return-type)
                  (if (string=? "const" head) ; discard cons qualifier
                      ...
                      (cons (list return-type: (string->symbol head))
                 ...
                 ((searching-parameters)
                  (cond ((string=? "const" head) ; discard const qualifier
                        ...
                        ((char=? #\( (string-ref head 0)) ; deal with (
                         (if (and (> head-len 6) (string=? "(void)" (substring head 0 6)))
                             (list (list parameters: '()))
                             (recur (cons (substring head 1 head-len)
                                          tail)
                                    'searching-parameters)))
                        (else (cons (list parameter: (string->symbol head))
  ...
  (let* ((tokens (tokenize declaration))
         (sl (analyze tokens)))
    (if (not (null? sl))
        (pp `(define ,(string->symbol (cadr (assq function-name: sl)))
...
(for-each generate-function-binding (call-with-input-string gl-es2-api (lambda (p) (read-all p read-line))))
